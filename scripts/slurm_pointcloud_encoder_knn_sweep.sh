#!/usr/bin/env bash
#SBATCH --job-name=pc_encoder_knn
...
#SBATCH --array=0-8%3
#SBATCH --output=slurm_pointcloud_encoder_knn_%A_%a.out
#SBATCH --error=slurm_pointcloud_encoder_knn_%A_%a.err
...
uv run --no-sync --project "$REPO" python -m examples.pointcloud.encoder_knn_sweep run \
