#!/usr/bin/env bash
#SBATCH --job-name=pc_class_rotate
...
#SBATCH --array=0-2%3
#SBATCH --output=slurm_pointcloud_new_class_rotation_%A_%a.out
#SBATCH --error=slurm_pointcloud_new_class_rotation_%A_%a.err
...
uv run --no-sync --project "$REPO" python -m examples.pointcloud.new_class_sweep rotate-test \
